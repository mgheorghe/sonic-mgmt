#!/bin/bash
# Manual 1-ENI load on Nvidia DPU0 (000apl + 000eni + 000map).
# Bypasses the pytest framework (intermittent EOF on first push) and uses the
# same optimized gnmi-agent stack: mounted gnmi_client.py + go_gnmi_utils.py +
# proto_utils.py with GNMI_NOTLS=1, --no-proto, --batch_val 1000.
...
T1=$(date +%s.%N)
awk -v a="$T0" -v b="$T1" 'BEGIN { printf "WALL[TOTAL 1-ENI LOAD on DPU0]: %.3fs\n", (b - a) }' < /dev/null

echo
echo "=== HGETALL COUNTERS_ENI_NAME_MAP on DPU0 (via :5021) ==="
sshpass -p password ssh -o StrictHostKeyChecking=no -p $SSH_PORT admin@$NPU_IP \
  'sudo sonic-db-cli COUNTERS_DB HGETALL COUNTERS_ENI_NAME_MAP' 2>&1 | tail -3

echo
echo "=== DPU_APPL_DB DBSIZE on DPU0 ==="
sshpass -p password ssh -o StrictHostKeyChecking=no -p $SSH_PORT admin@$NPU_IP \
