#!/usr/bin/env python3
"""
Jinja2-based generator for DASH Private-Link config files.
Produces the same JSON output as dpugen/dash.py but via readable Jinja2 templates.

Usage:
    python render.py                          # output to ./output/
    python render.py -o pl_100c              # output to ./pl_100c/
    python render.py -p ../pl_100c/dflt_params.py   # use custom params

Templates:
    templates/apl.json.j2  - Appliance + routing-type (1 per DPU)
    templates/eni.json.j2  - VNet + ENI + route-group + routes (1 per ENI)
    templates/map.json.j2  - VNet mappings + ENI-route binding (1 per ENI)
"""
...
def network10(ip_int_val):
    """IP integer -> 'x.y.0.0/10' network string."""
    return str(ipaddress.IPv4Network(f'{filt_ipv4(ip_int_val)}/10', strict=False))
...
#  Mirrors OutRouteRules.items() per-ENI: builds a block-size mix that tiles
#  ips_per_nsg into non-summarizable decompositions summing to target_per_nsg.
# ============================================================================
...
def compute_outbound_routes(ip_r_start_eni, eni_index, params, total_outbound_routes):
    """
    Yield the outbound route dicts for a single ENI.

    Each yielded dict has: ip (str), mask (int), routing_type (str),
...
def generate(params, output_dir, prefix='pl_100'):
    ...
    # Matches dash.py's per-ENI TOTAL_OUTBOUND_ROUTES split:
    # conf_total // DPUS // enis_per_dpu  ==  conf_total // ENI_COUNT
